...
get_all_sizes(char * location, unsigned long nbr_of_jpgs){
	...
	for(;count<=nbr_of_jpgs;count++){
		...
		sprintf_s(filename, sizeof(filename), "echo \"get all sizes: nbrjpgs: %lu, count: %lu \" >> info.info", nbr_of_jpgs, count);
		system(filename);

		unsigned long len;
		memset(filename, '\0',128);
		sprintf_s( filename, sizeof(filename), "%lu.jpg", count);
		ret = fopen_s( &fp, filename, "rb");
		//fp = fopen(filename, "r");

		if(!fp){
			system("echo \"could not open .jpg\" > avigarage_error.info");			
			continue;
		}
		fseek(fp, 0, SEEK_END);
		len = ftell(fp);
	...
	sprintf_s(filename, sizeof(filename), "echo \"returning size: %lu\" >> info.info", sizes);
	system(filename);
...
output_AVI_file(FILE * file_ptr, char * id, char * resolution, char * location, unsigned long fps, unsigned long nbr_of_jpgs){
	...
	errno_t ret = 0;
	memset(path,'\0',128);
	sprintf_s(path, sizeof(path), "%s",location);
	chdir(path);

	FILE * fp;

	ret = fopen_s( &fp, "1.jpg", "rb");
	// fp = fopen("1.jpg", "r");
	fseek(fp, 0, SEEK_END);
	...
	strcpy_s(jpg_resolution, sizeof(jpg_resolution), resolution);
...
char resolution_default[64]; //  = "800x600";
char location_default[64]; // = "jpgs";
char fps_default[64]; //  = "10";
char nbrjpgs_default[64]; // = "5";
...
{
	errno_t ret = 0;
	char * resolution = resolution_default; //  "800x600";
	char * location = location_default; //  "jpgs";
	char * fps = fps_default; //  "10";
	char * nbrjpgs = nbrjpgs_default; //  "5";

	strcpy_s(resolution_default, sizeof(resolution_default), "800x600" );
	strcpy_s(location_default, sizeof(location_default), "jpgs");
	strcpy_s(fps_default, sizeof(fps_default),"10" );
	strcpy_s(nbrjpgs_default, sizeof(nbrjpgs_default), "5");
